          ...
          ">
                    ...
                    <v-row style="align: center; justify-content: center">
                        ...
                        <v-col cols="2">
                            ...
                            <v-row v-if="item.descuento > 0" style="justify-content: center">${{
                                cantidad[i] *
                                (item.precioVenta - (item.precioVenta * item.descuento) / 100)
                            }}</v-row>
                ...
                <v-card class="mx-auto" min-height="100%">
                    ...
                    <v-card-text>
                        <v-row class="my-4 text-subtitle-1">
                            <v-col cols="5"><b>Subtotal</b></v-col>
                            <v-col style="text-align: right" cols="5"><b>${{ subtotal }}</b></v-col>
                        </v-row>
                        <v-divider></v-divider>
                        ...
                        <div v-if="radios == 'local'">
                            <v-row class="my-4 text-subtitle-1">
                                <v-col cols="5"><b>10% Descuento</b></v-col>
                                <v-col cols="5" style="text-align: right"><b>-$ {{ 0.1 * subtotal }}</b></v-col></v-row>
                            <v-row class="my-4 text-subtitle-1">
                                <v-col cols="5"><b>Total</b></v-col>
                                <v-col cols="5" style="text-align: right"><b>${{ subtotal * 0.9 }}</b></v-col></v-row>
                        </div>
                    </v-card-text>

                    <v-card-text>
                        <v-row v-if="(subtotal > 0 && radios == 'local') || pedido.domicilioid != ''
                            " style="justify-content: center; margin: 5%" dense>
...
} from "@/funciones/ControlStock.js";
export default {
    data() {
        return {
    ...
    methods: {
                ...
                try {
                    if (JSON.parse(localStorage.getItem(clave)).cantidad) {
                        this.getmanufacturados(
                            clave,
                            Number(JSON.parse(localStorage.getItem(clave)).cantidad)
                        );
                        carrovacio = false;
                    }
                } catch (err) {
        ...
        async agregarProducto(item, j, index) {
            ...
            if (this.cantidad[index] == 0) {
                ...
                localStorage.setItem(
                    item._id,
                    JSON.stringify({ cantidad: this.cantidad[index] })
                );
                if (contStockauxiliar != this.conStock[index]) {
                    //detecto variaciones del conStock para que me habilite/dehabilite el agregar mas articulos
        ...
        async calculaSubtotal() {
            ...
            for (let i = 0; i < this.items.length; i++) {
                if (this.items[i].descuento > 0) {
                    this.subtotal +=
                        (this.items[i].precioVenta -
                            (this.items[i].precioVenta * this.items[i].descuento) / 100) *
                        this.cantidad[i];
                } else {
        ...
        async registrarPedido() {
            ...
            if (this.pedido.tipoEnvio == "delivery") {
                ...
                this.pedido.domicilioid = this.domicilioElegido;
            } else {
