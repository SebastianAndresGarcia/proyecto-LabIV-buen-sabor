...
<template>
  <div class="app">
    <v-app-bar
      ...
      color="black"
      dark
      v-if="!pathname.includes('/administrador')"
    >
      ...
      <div v-if="!pathname.includes('/micarrito')" style="margin: 10px">
        <!-- si navega en la vista del carrito, no lo muestra en la navbar -->
      ...
      <div v-else style="justify-content: center; margin: 10px">
        <v-btn
          ...
          elevation="20"
          rounded
          href="http://localhost:8080/ManufacXrubro/0"
          color="success"
        ...
        <v-tabs align-with-title>
          <v-tab>
            <v-btn
              ...
              block
              text
              href="http://localhost:8080/#carousel"
              >Ofertas del Día</v-btn
            >
          </v-tab>
              ...
              <v-list-item v-for="(item, index) in rubros" :key="index">
                <v-list-item-title
                  ><v-btn
                    block
                    text
                    :href="'http://localhost:8080/ManufacXrubro/' + item._id"
          ...
          <v-tab>
            <v-btn
              ...
              block
              text
              href="http://localhost:8080/#dondeestamos"
              >Sobre Nosotros</v-btn
            >
          </v-tab>
          <v-tab v-if="showAdminBoard" href="/administrador">
            Administrador
          </v-tab>
        </v-tabs>
      </template>
        ...
        <v-list-item-group>
          <v-list-item
            ><a
              ...
              href="http://localhost:8080/MisCompras"
              >Mis Compras</a
            ></v-list-item
          >
...
export default {
  ...
  methods: {
    async getRubrosGeneral() {
      const res = await fetch("http://localhost:3000/rubrosgeneral");
      const resJson = await res.json();
      this.rubros = resJson;
      const respuesta = await fetch("http://localhost:3000/rubrosInsumos");
